...
\begin{theorem}[有限世界集合的基数上界定理]
若固定一个含 $N$ 个候选个体的集合与一个含 $M$ 个候选命题的集合，并只考虑由它们生成的世界，则 $\mathcal{W}_{N,M}=\{w\in\mathcal{W}_0\mid |D_w|\leq N,\,|\Phi_w|\leq M\}$ 是有限集，其基数的一个上界为：
$$|\mathcal{W}_{N,M}| \leq \sum_{d=1}^{N} \sum_{p=0}^{M} \binom{N}{d} \cdot \binom{M}{p} \cdot 2^{p}$$
其中 $\binom{M}{p}$ 表示从 $M$ 个命题中选择 $p$ 个命题的组合数。
\end{theorem}
...
\begin{theorem}[世界不可混淆性定理（World Non-Confusability）]
...
\begin{theorem}[世界的哈斯图]
世界图 $G=(\mathcal{W}_0,E)$ 的**哈斯图（Hasse Diagram）**是一个简化的有向图，去掉了所有可由传递性推导出的边。即若存在 $w_1,w_2,w_3\in\mathcal{W}_0$ 使得 $w_1 \prec w_2$ 且 $w_2 \prec w_3$，则在哈斯图中不画出边 $(w_1,w_3)$，并省略箭头。若 $w_1 \prec w_2$，则在哈斯图中画出一条从 $w_1$ 指向 $w_2$ 的线段，且 $w_2$ 位于 $w_1$ 的**上方**。
\end{theorem}

### 极小元与层函数

\begin{theorem}[良基子集的极小元存在]
...
- 若 $|R_G| = 0$，则 $G$ 是真正的**神（God）**。
- 若 $0 < |R_G| \ll |\Phi_{w_G}|$，则 $G$ 是**近乎全能者（Nearly Omnipotent Being）**。
- 若 $|R_G| \approx |\Phi_{w_G}|$，则 $G$ 是**极度受限者（Extremely Restricted Being）**。
\end{theorem}
